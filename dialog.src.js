...
(function ($, window, undefined) {
     ...
     */
        ...
        _path = (function (script, i, me) {
            return "http://static.tcy365.com/cdn/dialog/1.3/";
            var l = script.length;
    ...
    var G_ZIndex = {
        set: function (n) {
            _top.lhgdialog.zIndex = n;
        },
        get: function () {
            return _top.lhgdialog.zIndex;
        },
        init: function () {
            if (typeof(_top.lhgdialog) == "undefined") {
                _top.lhgdialog = {};
                _top.lhgdialog.zIndex = 1976;
            }
        }
    };
    var G_list = {
        set: function (id, config) {
            _top.lhgdialog.list[id] = config;
            return config;
        },
        get: function (id) {
            if (_top.lhgdialog.list[id]) {
                return _top.lhgdialog.list[id];
            }
            return false;
        },
        getAll: function () {
            return _top.lhgdialog.list;
        },
        init: function () {
            if (typeof(_top.lhgdialog.list) == "undefined") {
                _top.lhgdialog.list = {};
            }
        }
    };
    ...
    var lhgdialog = function (config) {
        ...
        api = G_list.get(config.id);
        if (api) {
            return api._showFlag = "hide" ? api.show() : api._init(config);
        }
        ...
        G_ZIndex.set(config.zIndex);


        _count++;

        return G_list.set(config.id, new lhgdialog.fn._init(config));
    };
    ...
    lhgdialog.fn = lhgdialog.prototype = {
        ...
        _init: function (config) {
            ...
            that.title(config.title).content(content, true, isIfr).size(config.width, config.height).position(config.left, config.top).time(config.time)[config.show ? 'show' : 'hide'](true).zindex(1);
        ...
        zindex: function (n) {
            var that = this,
                ...
                load = that._load,
                index;
            lhgdialog.focus = that.config.id;
            if (n != 1 && G_list.get(that.config.id) && G_list.get(that.config.id).DOM.wrap[0].style.zIndex == G_ZIndex.get()) {
                return that;
            }
            G_ZIndex.set(G_ZIndex.get() + 10);
            index = G_ZIndex.get();
            ...
            // 设置最高层的样式
            for (var i in G_list.getAll()) {
                G_list.get(i).DOM.border.removeClass('ui_state_focus');
                if (G_list.get(i)._load) {
                    G_list.get(i)._load.style.display = '';
                }
            }
        ...
        lock: function () {
            ...
            if (!mask) {
                ...
                $(mask).css({
                    ...
                    background: config.opacityColor
                });
                style = mask.style;
                if (_ie6) {
                    mask.innerHTML = frm;
        ...
        unlock: function (flag) {
            var that = this,
                config = that.config,
                mask = $('#ldg_lockmask', _doc)[0];
            var obj = {id: 0, zindex: 0}, mZindex;
            //console.log(G_list.get(config.id)._lock);
            if (mask && that._lock) {
                for (var i in G_list.getAll()) {
                    if (config.id != i) {
                        //console.log(1);
                        if (G_list.get(i)._lock) {
                            mZindex = parseInt(G_list.get(i).DOM.wrap[0].style.zIndex);
                            if (mZindex > obj.zindex) {
                                obj.zindex = mZindex;
                                obj.id = i;
                            }
                        }
                ...
                if (obj.id != 0) {
                    mask.style.zIndex = obj.zindex - 1;
                } else {
        ...
        close: function () {
            ...
            var mZindex = 0, mId = 0;
            for (var i in G_list.getAll()) {
                if (mZindex < parseInt(G_list.get(i).DOM.wrap[0].style.zIndex)) {
                    mZindex = parseInt(G_list.get(i).DOM.wrap[0].style.zIndex);
                    mId = i;
                }
            ...
            if (mId != 0) {
                lhgdialog.focus = mId;
                G_list.get(lhgdialog.focus).zindex();
            } else {
        ...
        get: function (id, object) {
            if (G_list.get(id)) {
                if (object === 1) {
                    return G_list.get(id);
                } else {
                    return G_list.get(id).iwin || null;
                }
            }
        ...
        _iframe: function (url) {
            ...
            var that = this,
                ...
                initCss = 'position:absolute;left:-9999em;border:none 0;background:transparent;width:1px;height:1px; overflow:hidden;',
            ...
            var load = that._fmLoad = function () {
                ...
                try {
                    iwin = that.iwin = iframe.contentWindow; // 定义窗口对象content属性为内容页的window对象
                    $idoc = $(iwin.document);
                ...
                iHeight = config.height === 'auto' ? $ibody.height() : config.height;
        ...
        _ie6PngFix: function () {
            if (_ie6) {
                var i = 0,
                    elem, png, pngPath, runtimeStyle, path = lhgdialog.setting.path + '/skins/',
    ...
    _use = function (event) {
        var limit, startWidth, startHeight, startLeft, startTop, isResize, api = G_list.get(lhgdialog.focus),
            config = api.config,
            DOM = api.DOM,
            wrap = DOM.wrap[0],
            title = DOM.title,
    ...
    $(function () {
        ...
        loadImg.src = _path + 'skins/icons/loading.gif';
        ...
        lhgdialog.setting.extendDrag && (function (dragEvent) {
            ...
            dragEvent.start = function () {
                var api = G_list.get(lhgdialog.focus),
                    main = api.DOM.main[0],
                    iframe = api.iframe;

                dragEvent._start.apply(this, arguments);
                style.display = 'block';
                style.zIndex = G_ZIndex.get() + 3;
...
(function ($, lhgdialog, undefined) {
    ...
    lhgdialog.closeAll = function () {
        for (var i in G_list.getAll()) {
			(function(str){
				$.dialog.list[str].close();
			})(i);            
        }
    };
    /**
